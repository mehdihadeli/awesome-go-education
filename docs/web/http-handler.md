# Http Handler

## 📕 Articles
- [Handler - golang.org](https://pkg.go.dev/net/http?utm_source=godoc#Handler)
- [Demystifying HTTP Handlers in Golang](https://medium.com/geekculture/demystifying-http-handlers-in-golang-a363e4222756)
- [An Introduction to Handlers and Servemuxes in Go](https://www.alexedwards.net/blog/an-introduction-to-handlers-and-servemuxes-in-go)
- [Revisiting HTTP Handlers](https://quii.gitbook.io/learn-go-with-tests/questions-and-answers/http-handlers-revisited)
- [Handling requests](https://livebook.manning.com/book/go-web-programming/chapter-3/29)
- [An Overview of Server-Side HTTP APIs in Go](https://betterprogramming.pub/overview-of-server-side-http-apis-in-go-44f052737e4b)
- [Golang HTTP Handlers](https://blog.bitclvx.com/post/golang-handler/)
- [The http.Handler wrapper technique in #golang UPDATED](https://medium.com/@matryer/the-http-handler-wrapper-technique-in-golang-updated-bc7fbcffa702)
- [The http.Handler Interface](https://lets-go.alexedwards.net/sample/02.10-the-http-handler-interface.html)
- [Why Cant I Pass This Function as an Http Handler?!](https://www.calhoun.io/why-cant-i-pass-this-function-as-an-http-handler/)
- [HandlerFunc, Handle and DefaultServeMux in Go](https://echorand.me/posts/golang-dissecting-listen-and-serve/)