# Method

## 📕 Articles
- [Methods - Practical Go Lessons](https://www.practical-go-lessons.com/chap-14-methods)
- [Go Methods Tutorial](https://tutorialedge.net/golang/go-methods-tutorial/)
- [Defining Methods in Go](https://www.digitalocean.com/community/tutorials/defining-methods-in-go)
- [Anatomy of methods in Go](https://medium.com/rungo/anatomy-of-methods-in-go-f552aaa8ac4a)
- [Go Best Practices: Pointer or value receivers?](https://flaviocopes.com/golang-methods-receivers/)
- [Should I define methods on values or pointers?](https://go.dev/doc/faq#methods_on_values_or_pointers)
- [Pointer vs. value receiver](https://yourbasic.org/golang/pointer-vs-value-receiver/)
- [GOLANG TUTORIAL - VALUE OR POINTER RECEIVER](https://www.bogotobogo.com/GoLang/GoLang_Receiver_Value_vs_Pointer.php)
- [Value receiver vs. pointer receiver](https://newbedev.com/value-receiver-vs-pointer-receiver)
- [Pointer vs Value Receiver in methods while implementing an interface in Go (Golang)](https://golangbyexample.com/pointer-vs-value-receiver-method-golang/)
- [Summary to the difference between T and T* method sets in Go](https://gronskiy.com/posts/2020-04-golang-pointer-vs-value-methods/)
- [GOLANG method set](https://programmerall.com/article/56281522430/)
- [GOLANG different method set - Golang.org](https://golang.org/doc/faq#different_method_sets)
- [Method Values - Golang.org](https://go.dev/ref/spec#Method_values)
- [Method Sets - Golang.org](https://golang.org/ref/spec#Method_sets)
- [Interface Types - Golang.org](https://go.dev/ref/spec#Interface_types)
- [Things about the golang | method set](https://developpaper.com/things-about-the-golang-method-set/)
## 📺 Videos
- [Go Syntax - Method Sets](https://www.youtube.com/watch?v=Z5cvLOrWlLM)