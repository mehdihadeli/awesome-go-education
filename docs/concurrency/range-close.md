# Range-Close

## 📕 Articles
- [For-range loop for a channel in Go (Golang)](https://golangbyexample.com/for-range-loop-channel-go/)
- [Range channel](https://golangr.com/range-channel/)
- [Range over channels in Go](https://medium.com/golangspec/range-over-channels-in-go-3d45dd825d2a)
- [How to Manage Go Channels With Range and Close](https://betterprogramming.pub/manging-go-channels-with-range-and-close-98f93f6e8c0c)
- [Golang Channel Range And Close Channel](https://hackthedeveloper.com/golang-channel-range-close/)
- [Channels - golangbot](https://golangbot.com/channels/)
- [Go by Example: Range over Channels](https://gobyexample.com/range-over-channels)
- [(Almost) ranging over multiple Go channels simultaneously](https://rillabs.org/posts/range-over-multiple-go-channels)